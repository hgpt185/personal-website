...
The rough heuristic I use now comes from HikariCP's documentation: `pool size = (core count x 2) + effective spindle count`. It's a reasonable starting point for CPU-bound workloads. For I/O-heavy services, the calculation shifts because threads are often just idle, waiting on database round-trips, which means you can support more concurrency.
